...
class ActivityLessonDetails : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null && intent.getParcelableExtra("LESSON", Lesson::class.java) != null) {
            // coming from AdapterLessons:
            lesson = intent.getParcelableExtra("LESSON", Lesson::class.java)!!
            findPhoneNumbersByStudentId()
        } else if (savedInstanceState == null && intent.getParcelableExtra("UPDATED_LESSON", Lesson::class.java) != null) {
            // coming from ActivityEditLesson:
            lesson = intent.getParcelableExtra("UPDATED_LESSON", Lesson::class.java)!!
            findPhoneNumbersByStudentId()
        } else if (savedInstanceState != null) {
            // after screen rotation:
            progressBar.visibility = View.GONE
            lesson = savedInstanceState.getParcelable("SAVED_LESSON", Lesson::class.java)!!
        ...
        val timeFrom = Functions.dateTimeFormatter.format(lesson.timeFrom)
        val timeTo = Functions.dateTimeFormatter.format(lesson.timeTo)
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray) {
        when (requestCode) {
            ActivityStudentDetails.PERMISSION_REQUEST_CALL_PHONE -> if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                dial(number)
            } else {
            ...
            if (!ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.CALL_PHONE)) {
                showMessageOKCancel { _, _ ->
                    ActivityCompat.requestPermissions(
                        ...
                        ActivityStudentDetails.PERMISSION_REQUEST_CALL_PHONE
                    )
                }
                return
            }
            ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.CALL_PHONE), ActivityStudentDetails.PERMISSION_REQUEST_CALL_PHONE)
            return
        }
